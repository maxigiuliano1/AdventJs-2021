 ...
 * El primero es el número de horas que tenemos disponible ⏳ y el segundo es un array donde cada índice es un curso y el valor el tiempo 
 * que se tarda en completar.
 * 
 * Tenemos claro que queremos hacer dos cursos así que la función debe devolver un array con el índice de los dos cursos que vamos a poder
 *  completar con el tiempo disponible proporcionado y usando el máximo tiempo disponible. Si no nos da tiempo, devolvemos null
 * 
...
function fixFiles(files) {
    let equals = '', repeat = 0, result = []
    for(let i = 0; i < files.length; i++){
      equals = files[i]
      for(let j = 1; j < files.length; j++){
        if(equals == files[j]){
          repeat++
          let variable = `${equals}(${repeat})`
          if(result.includes(variable)){
            var indice = result.indexOf(variable);
            result.splice(indice)
            break
          }
          if(result.includes(equals)){
