...
function shouldBuyFidelity(times) {
    let result = false, descuento = 0.75, tarjetaFidelidad = 250
    const ticket = 12, normal = times * ticket
    for(let i = 0; i < times; i++){
        let valor = (ticket * descuento)
        descuento *= 0.75
        tarjetaFidelidad += valor
    }
    return tarjetaFidelidad < normal ? result = true : result = false
  }
