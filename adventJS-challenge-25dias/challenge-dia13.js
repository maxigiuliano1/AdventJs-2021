...
function wrapGifts(gifts) {
    ...
    let finalGift = []
    let stringLength = gifts.map(g => g.length)
    let maxLengthString = Math.max(...stringLength)
    let boxCover = '*'.repeat(maxLengthString + 2)
    gifts.forEach(g => finalGift.push(`*${g}*`))
    return [boxCover,...finalGift,boxCover]
  }